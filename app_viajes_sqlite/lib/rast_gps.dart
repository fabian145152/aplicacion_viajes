...
import 'services/ubicacion_services.dart'; // 🔥 Servicio con SQLite
...
class _BotonCoordenadasState extends State<BotonCoordenadas>
  ...
  // 🔥 Actualiza estado: le avisa al UbicacionService (él hace todo)
  void actualizarEstado(EstadoChofer nuevoEstado) {
    if (_estadoActual != nuevoEstado) {
      ...
      // 🔥 El servicio se encarga de obtener GPS, guardar en SQLite y enviar
      _ubicacionService.actualizarEstado(
        _getStatusString(_estadoActual),
        viajeId: _viajeActualId ?? 0,
      );
    }
  }
  ...
  void initState() {
    ...
    // 🔥 Iniciar el servicio de ubicación con SQLite
    // (esto normalmente ya se hizo en login.dart, pero por las dudas)
    if (!_ubicacionService.isRunning) {
      _ubicacionService.iniciar(widget.numeroMovil, widget.nombreChofer);
    }
  ...
  void _asignarViaje(Map<String, dynamic> viaje) async {
    ...
    // 🔥 Cambiar estado a ASIGNADO (envía ubicación con viaje_id)
  ...
  void _toggleTracking(bool active) async {
    if (active) {
      ...
      _viajeActualId = null;
      // 🔥 Cambiar a ACTIVO (el servicio envía ubicación automáticamente)
      actualizarEstado(EstadoChofer.activo);
